...
export default function ProductDetailPage() {
    ...
    async function fetchProduct() {
      ...
      try {
        ...
        if ((response.data as any)?.success) {
          setProduct((response.data as any)?.data);
          const firstVariant = (response.data as any)?.data.variants.find((v: any) => v.status === 'active' && v.stockQuantity > 0
          );
          if (firstVariant) {
          ...
          if ((response.data as any)?.data.categories?.length > 0) {
            const categorySlug = (response.data as any)?.data.categories[0].slug;
            const relatedResponse = await productsApi.getProducts({ category: categorySlug, limit: 4 });
            const related = (relatedResponse.data as any)?.data?.data.filter((p: any) => p.id !== (response.data as any)?.data.id);
  ...
  useEffect(() => {
    if (product && selectedSize && selectedColor) {
      const variant = product.variants.find((v: any) => v.size?.label === selectedSize && v.color?.displayName === selectedColor && v.status === 'active'
      );
      if (variant) {
  ...
  const colors = [...new Map(activeVariants.map((v: any) => [v.color?.name, v.color])).values()];
  const sizes = [...new Map(activeVariants.filter((v: any) => v.color?.name === selectedColor || !selectedColor).map((v: any) => [v.size?.label, v.size])).values()];

  const productImages = product.media?.filter((m: any) => m.type === 'image') || [];
  const primaryImage = productImages.find((m: any) => m.isPrimary) || productImages[0];
          ...
          <div className="space-y-6">
            ...
            {sizes.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {sizes.map((size: any) => {
                    const hasStock = activeVariants.some((v: any) => v.size?.label === size.label && v.color?.name === selectedColor && v.stockQuantity > 0);
